...
public class KeyTestView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		while (i < mKeyArray.size()) {
			...
			canvas.drawRect(current_x, current_y, current_x + per_width,
					current_y + per_height, mPaint);
			canvas.drawText(mKeyArray.get(i).getKeyName(), current_x, current_y
					+ per_height / 2 + 5, mWordPaint);
			i++;
			if (i % ROW == 0) {
				current_x = PER_GAP;
