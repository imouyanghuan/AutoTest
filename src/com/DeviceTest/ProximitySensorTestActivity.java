...
public class ProximitySensorTestActivity extends Activity{
	...
	private void init(){
		...
		if(illuminationSensor==null){
			...
			proximitysensorTV.setText(getString(R.string.proximitysensor_distance) + "1.0");//（数据没变化表示失败）");
	...
	private SensorEventListener sensorEventListener=new SensorEventListener() {
		...
		public void onSensorChanged(SensorEvent event) {
			...
			proximitysensorTV.setText(getString(R.string.proximitysensor_distance) + values[0]);
			//proximitysensorTV.setText(getString(R.string.proximitysensor_distance) + (values[0]>0?"离开":"接近"));// +"\n幅度："+illuminationSensor.getMaximumRange());
			}
		}
