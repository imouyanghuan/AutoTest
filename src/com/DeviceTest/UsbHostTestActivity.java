...
public class UsbHostTestActivity extends Activity {
	...
	private static final File HOST_STORAGE_DIRECTORY = new File(
			DeviceTest.usb_path);
	...
	public static String getHostStorageState() {
		try {
			return mStorageManager.getVolumeState(DeviceTest.usb_path);
		} catch (Exception rex) {
			rex.printStackTrace();
	...
	public void testReadAndWrite() {
		...
		if (true/* dotestReadAndWrite() */) { // 修改 ZZP ，MTK 系统用户对sdcard USBhost
												// 等无写权限
	...
	private boolean dotestReadAndWrite() {
		...
		Log.v("ZZP", ">>>>>>>>>>dotestReadAndWrite directoryName : "
				+ directoryName);
		...
		if (!directory.isDirectory()) { // Create Test Dir
			if (!directory.mkdirs()) {
				sBuilder.append(
						getString(R.string.MakeDir) + FAIL + " , path : "
								+ directoryName).append("\n");
				return false;
			} else {
				sBuilder.append(
						getString(R.string.MakeDir) + SUCCESS + " , path : "
								+ directoryName).append("\n");
			}
		}
		...
		try {
			...
			if (!f.createNewFile()) { // Create Test File
				...
				sBuilder.append(getString(R.string.CreateFile) + SUCCESS)
						.append("\n");
				...
				if (doReadFile(f.getAbsoluteFile().toString()).equals(
						TEST_STRING)) {
					sBuilder.append(getString(R.string.Compare))
							.append(SUCCESS).append("\n");
				} else {
					sBuilder.append(getString(R.string.Compare)).append(FAIL)
							.append("\n");
					return false;
				}
			}

			sBuilder.append(getString(R.string.FileDel))
					.append((f.delete() ? SUCCESS : FAIL)).append("\n");
			sBuilder.append(getString(R.string.DirDel))
					.append((directory.delete() ? SUCCESS : FAIL)).append("\n");
			return true;
	...
	StorageEventListener mStorageListener = new StorageEventListener() {
		...
		public void onStorageStateChanged(String path, String oldState,
				String newState) {
			if (path.equals(DeviceTest.usb_path)
