...
public class UsbHostTestActivity extends Activity {
    ...
    private static final File HOST_STORAGE_DIRECTORY = new File(DeviceTest.usb_path);
    ...
    public void testReadAndWrite() {
        ...
        if (true/*dotestReadAndWrite()*/) { // 修改 ZZP ，MTK 系统用户对sdcard USBhost 等无写权限
    ...
    private boolean dotestReadAndWrite() {
        ...
        Log.v("ZZP", ">>>>>>>>>>dotestReadAndWrite directoryName : " + directoryName);
        
        File directory = new File(directoryName);
        if (!directory.isDirectory()) { // Create Test Dir
            if (!directory.mkdirs()) {
                sBuilder.append(getString(R.string.MakeDir) + FAIL + " , path : " + directoryName).append("\n");
                return false;
            } else {
                sBuilder.append(getString(R.string.MakeDir) + SUCCESS + " , path : " + directoryName).append(
                        "\n");
            }
        }
    ...
    StorageEventListener mStorageListener = new StorageEventListener() {
        ...
        public void onStorageStateChanged(String path, String oldState, String newState) {
        	if (path.equals(DeviceTest.usb_path) && newState.equals(Environment.MEDIA_MOUNTED)) {
                if (testDevDevice()) {
                    testHostStorageState();
                    testReadAndWrite();
                    mPassBtn.setVisibility(View.VISIBLE);
