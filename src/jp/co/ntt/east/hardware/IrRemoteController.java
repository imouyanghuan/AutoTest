...
public class IrRemoteController {
	...
	public void send(IrRemoteController.Data[] data, int timeout)
			throws IOException {
		send(data, timeout, -1);
	}

	public void send(IrRemoteController.Data[] data, int timeout, int count)
			throws IOException {
		if (isRunning) {
	...
	private static native int native_hal_set_formate(int high, int low,
			byte[] data, int length, int duration, int startHigh, int startLow,
			int stopHigh, int data0Pattern, int data0High, int data0Low,
			int data1Pattern, int data1High, int data1Low, int count);
	...
	private class SendThread extends Thread {
		...
		public void run() {
			...
			for (int i = 0; i < data.length && isRunning; i++) {
				Data d = data[i];
				native_hal_set_formate(d.carry_high, d.carry_low, d.data,
						d.length, d.duration, d.startHigh, d.startLow,
						d.stopHigh, d.data0Pattern, d.data0High, d.data0Low,
						d.data1Pattern, d.data1High, d.data1Low, d.count);
				native_hal_send();
			}
